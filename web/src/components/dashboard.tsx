...
export function Dashboard({ bootstrap }: { bootstrap: Bootstrap }) {
  ...
  const loadSummary = useCallback(async () => {
    ...
    const params = new URLSearchParams({ organizationId: bootstrap.organization.id, storeId: apiScope, from: from.toISOString(), to: new Date().toISOString() });
    try {
      const response = await fetch("/api/analytics/summary?" + params);
      const result = await response.json().catch(() => null);
      if (!response.ok) { setMessage(result?.error?.message ?? "数据加载失败"); return; }
      setSummary(result);
      await offlineDb.dashboardSnapshots.put({ key: bootstrap.organization.id + ":" + apiScope, payload: JSON.stringify(result), updatedAt: Date.now() });
    } catch {
      const snapshot = await offlineDb.dashboardSnapshots.get(bootstrap.organization.id + ":" + apiScope);
  ...
  }, [bootstrap.organization.id, apiScope]);
  ...
  return <main className="app-shell">
    ...
    {loading || !summary ? <section className="loading-card">正在整理账目…</section> : <>
      <section className="hero-card"><p>最近 7 天净收益</p><strong>{formatFen(summary.netFen)}</strong><span>{summary.transactionCount} 笔收支 · {storeScope === "all" ? "已汇总可查看摊位" : selectedStore?.name}</span></section>
      ...
      <section className="panel"><div className="panel-title"><h2>近 7 天趋势</h2><span>收入 / 支出</span></div><TrendChart data={summary.trend} /></section>
