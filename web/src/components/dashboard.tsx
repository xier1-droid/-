...
type Category = { id: string; type: "INCOME" | "EXPENSE"; name: string };
type Bootstrap = { userId: string; organization: { id: string; name: string; timezone: string; currency: string }; member: { id: string; role: string }; stores: Store[]; categories: Category[] };
...
export function Dashboard({ bootstrap, initialDateRange }: { bootstrap: Bootstrap; initialDateRange: DateRange }) {
  ...
  const sync = useCallback(async () => {
    await flushPendingOperations(bootstrap.userId, bootstrap.organization.id);
    setSyncCount(await pendingCount(bootstrap.userId, bootstrap.organization.id));
    await loadSummary();
  }, [bootstrap.userId, bootstrap.organization.id, loadSummary]);
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => { void loadSummary(); void pendingCount(bootstrap.userId, bootstrap.organization.id).then(setSyncCount); }, 0);
    return () => window.clearTimeout(timer);
  }, [bootstrap.userId, bootstrap.organization.id, loadSummary]);
  ...
  return <main className="app-shell">
    ...
    {loading || !summary ? <section className="loading-card">正在整理账目…</section> : <>
      ...
      <section className="panel trend-panel"><div className="panel-title"><div><h2>{rangeLabel(dateRange)} 收支趋势</h2><span>按{summary.granularity === "day" ? "天" : summary.granularity === "week" ? "周" : "月"}汇总 · 收入 / 支出</span></div><button className="date-range-button" onClick={() => setShowDatePicker(true)}>{dateRange.mode} ›</button></div><TrendChart data={summary.trend} granularity={summary.granularity} /></section>
...
function EntryModal({ store, userId, organizationId, categories, onClose, onSaved, onQueued }: { store: Store; userId: string; organizationId: string; categories: Category[]; onClose: () => void; onSaved: () => void; onQueued: (message: string) => void }) {
  const [type, setType] = useState<"INCOME" | "EXPENSE">("INCOME"); const [category, setCategory] = useState(""); const [message, setMessage] = useState(""); const [pending, setPending] = useState(false);
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = { storeId: store.id, type, amountFen, paymentMethod: form.get("paymentMethod"), category, occurredAt: new Date(String(form.get("occurredAt"))).toISOString(), note: form.get("note") || null, clientOperationId: crypto.randomUUID() };
    if (!navigator.onLine) { await queueOperation({ id: payload.clientOperationId, userId, organizationId, url: "/api/ledger-entries", method: "POST", body: JSON.stringify(payload) }); onQueued("网络不可用，账目已安全保存在本机，恢复网络后会自动同步。"); return; }
  ...
  return <Modal title="记一笔" onClose={onClose}><form className="form-stack" onSubmit={submit}><div className="segmented"><button type="button" className={type === "INCOME" ? "selected income" : ""} onClick={() => { setType("INCOME"); setCategory(""); }}>收入</button><button type="button" className={type === "EXPENSE" ? "selected expense" : ""} onClick={() => { setType("EXPENSE"); setCategory(""); }}>支出</button></div><label>金额（元）<input required inputMode="decimal" name="amount" placeholder="例如 28.50" autoFocus /></label><label>收付款方式<select name="paymentMethod"><option value="WECHAT">微信</option><option value="ALIPAY">支付宝</option><option value="CASH">现金</option><option value="BANK_CARD">银行卡</option><option value="OTHER">其他</option></select></label><label>分类<input required value={category} onChange={(event) => setCategory(event.target.value)} placeholder="选择常用分类或直接输入" /></label><div className="category-suggestions">{categories.filter((item) => item.type === type).map((item) => <button type="button" className={category === item.name ? "active" : ""} key={item.id} onClick={() => setCategory(item.name)}>{item.name}</button>)}</div><label>发生时间<input required type="datetime-local" name="occurredAt" defaultValue={new Date().toISOString().slice(0, 16)} /></label><label>备注（选填）<input name="note" placeholder="例如：王阿姨预订" /></label>{message && <p className="form-error">{message}</p>}<button className="primary wide" disabled={pending}>{pending ? "保存中…" : "保存账目"}</button></form></Modal>;
}
