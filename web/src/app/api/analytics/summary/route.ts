...
import { getDayBounds, getSummary, type TrendGranularity } from "@/lib/analytics";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const toOrganizationDate = (value: Date) => new Intl.DateTimeFormat("en-CA", { timeZone: organization.timezone, year: "numeric", month: "2-digit", day: "2-digit" }).format(value);
    const today = toOrganizationDate(new Date());
    const fromDate = fromParam === "all" ? (firstEntry ? toOrganizationDate(firstEntry.occurredAt) : today) : (fromParam ?? toOrganizationDate(new Date(Date.now() - 6 * 86_400_000)));
    ...
    const { start: from } = getDayBounds(fromDate, organization.timezone);
    const toBounds = getDayBounds(toDate, organization.timezone); const to = new Date(toBounds.end.getTime() - 1);
    ...
    return NextResponse.json({ ...getSummary(entries, from, to, granularity, organization.timezone), recentEntries, storeIds, range: { from: fromDate, to: toDate } });
